...
- (id)initWithScriptView:(DKCanvasView *)scriptViewp width:(CGFloat)widthp height:(CGFloat)heightp {
	if( self = [super init] ) {
    
    NSLog(@"initWithScriptView");
        
//        CGSize a = scriptViewp.frame.size.
...
//        NSLog(@"vertexBuffer:%@", vertexBuffer);
		...
		fontCache = [EJFontCache instance];
...
//	[state->strokeObject release];
	state->strokeObject = strokeObject;
}

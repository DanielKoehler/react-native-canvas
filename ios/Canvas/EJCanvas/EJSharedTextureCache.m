...
+ (EJSharedTextureCache *)instance {
	if( !sharedTextureCache ) {
		sharedTextureCache = [EJSharedTextureCache new];
	}
...
- (id)init {
	if( self = [super init] ) {
		...
		textures = (NSMutableDictionary *)CFBridgingRelease(CFDictionaryCreateMutable(NULL, 8, &kCFCopyStringDictionaryKeyCallBacks, NULL));
	}
	return self;
}
